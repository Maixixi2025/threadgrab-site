"""7-gate validation + cross-language identity checks for the new article."""
...
for lang, slug in SLUGS.items():
    ...
    if not title_m:
        ...
        print(f'  G1 title {len(vis)} ok' if 30 <= len(vis) <= 60 and vis == EXPECTED_TITLE[lang] else f'  G1 title {len(vis)} mismatch')
